...
| **Vortex**            | `vortex`            | Unified, **recommended** toolset surfacing both Vortex context and Vortex analysis tools under one name (stdio only; Cloud needs combined org entitlement; Server needs both hubs entitled; `run_advanced_code_analysis` stays Cloud-only) |
...
On SonarQube Server, stdio lists Vortex context tools when both the CAG and A3S hubs are entitled. Vortex analysis (`run_advanced_code_analysis`) stays Cloud-only.
