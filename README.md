...
[![Docker pulls](https://img.shields.io/docker/pulls/sonarsource/sonarqube-mcp)](https://hub.docker.com/r/sonarsource/sonarqube-mcp)
[![GitHub stars](https://img.shields.io/github/stars/SonarSource/sonarqube-mcp-server?style=flat)](https://github.com/SonarSource/sonarqube-mcp-server)
...
SonarQube MCP Server implements the Model Context Protocol (MCP) to connect AI agents with code quality and security analysis from SonarQube Server or SonarQube Cloud.

This repository contains the server source. Its tools let agents query existing SonarQube results, inspect quality gates and findings, and analyze supported code snippets without leaving the agent workflow. The same SonarQube analysis applies whether the code was written by a developer or generated by an agent.

## What it does

- Queries SonarQube projects, quality gates, issues, security hotspots, coverage, and other analysis measures.
- Analyzes supported code snippets directly in the agent context.
- Gives agents repeatable, explainable findings they can use to review or improve code.
- Connects to SonarQube Server and SonarQube Cloud through scoped user credentials.
