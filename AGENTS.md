...
| Vortex (CAG + SQAA / A3S)                                    | Combined org entitlement; stdio only                       | Both hubs entitled; stdio only; nil UUID org for SQAA |
| Agentic readiness                                            | Org feature flag                                           | Not available                |

On Cloud and Server, Vortex is one product: CAG (context) and SQAA/A3S (analysis) share a single entitlement check. Both must be entitled or neither registers. Cloud uses `OrgFeatureEntitlements` against the org UUID. Server AND-merges `GET /api/v2/cag/cag-entitlement/{nil-uuid}` and `GET /api/v2/a3s/org-entitlement/{nil-uuid}`: a 404 from either hub is quiet (pre-LTA / hub absent); `hasEntitlement: false` logs a licence line; 503 is a failed check. Over-consumption (`hasEntitlement: true`, `allowed: false`) still starts the proxied CAG engine and registers `run_advanced_code_analysis` (POST `/api/v2/a3s/analyses` with the nil UUID as organization). When Vortex is on, local snippet analysis (`analyze_code_snippet`) and IDE-bridge tools (`analyze_file_list`, `toggle_automatic_analysis`) are not registered. `run_advanced_code_analysis` needs a workspace mount. `CAG`/`A3S` naming is deprecated in favor of `Vortex`; the existing toolset keys (`cag`, `analysis`) and tool names (e.g. `run_advanced_code_analysis`) are unchanged for backward compatibility. Defaults include `vortex`; `cag` and `analysis` are not enabled unless requested.
