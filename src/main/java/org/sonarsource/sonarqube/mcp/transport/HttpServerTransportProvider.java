...
public class HttpServerTransportProvider {
    ...
    this.mcpTransportProvider = HttpServletStatelessServerTransport.builder()
      ...
      .contextExtractor(request -> {
        ...
        if (toolsets != null && !toolsets.isBlank()) {
          contextBuilder.put(CONTEXT_TOOLSETS_KEY, toolsets);
        }
        if (readOnly != null && !readOnly.isBlank()) {
          contextBuilder.put(CONTEXT_READ_ONLY_KEY, readOnly);
        }
  ...
  /**
   * Returns a {@link CapturingTransport} that intercepts {@code setMcpHandler} to hold the
   * SDK-provided handler reference. Pass it to {@code McpServer.sync(...).build()}, then call
   * {@link CapturingTransport#installToolsListFilter} to wrap the captured handler.
   */
  public CapturingTransport getCapturingTransport() {
    return new CapturingTransport();
  }

  public class CapturingTransport implements McpStatelessServerTransport {
    private McpStatelessServerHandler sdkHandler;

    @Override
    public void setMcpHandler(McpStatelessServerHandler handler) {
      sdkHandler = handler;
      mcpTransportProvider.setMcpHandler(handler);
    }
    ...
    /**
     * Replaces the SDK's handler on the servlet transport with a {@link ToolsListFilteringHandler}.
     * Must be called after {@code McpServer.sync(this).build()} on the same thread.
     */
    public void installToolsListFilter(List<Tool> enabledTools) {
      if (sdkHandler == null) {
        throw new IllegalStateException("installToolsListFilter called before the SDK set its handler");
      }
      mcpTransportProvider.setMcpHandler(new ToolsListFilteringHandler(sdkHandler, enabledTools));
    }
  }
