...
public class StdioServerTransportProvider implements McpServerTransportProvider {
   ...
   * True in production (Docker containers), false in tests.
   */
  ...
  /**
   * Determines if we should exit on stdin EOF.
      ...
      if (isStarted.compareAndSet(false, true)) {
        this.inboundScheduler.schedule(() -> {
          ...
          try {
            ...
            // When stdin closes (EOF detected) in production,
            // terminate the JVM to ensure Docker containers exit properly.
