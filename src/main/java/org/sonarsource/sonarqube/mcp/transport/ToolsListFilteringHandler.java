 ...
 * responses and apply per-request narrowing based on the {@code SONARQUBE_TOOLSETS} and
 * {@code SONARQUBE_READ_ONLY} HTTP headers.
 *
 ...
 *
 * <p>All other MCP methods ({@code initialize}, {@code tools/call}, etc.) are delegated
...
public class ToolsListFilteringHandler implements McpStatelessServerHandler {
  ...
  public Mono<McpSchema.JSONRPCResponse> handleRequest(McpTransportContext transportContext, McpSchema.JSONRPCRequest request) {
    if (McpSchema.METHOD_TOOLS_LIST.equals(request.method()) && hasPerRequestFilter(transportContext)) {
