...
public class ExternalToolsLoader {
   ...
   * <p>
   * Each provider is attempted independently. If a provider's dependencies
   * are not available (e.g., command not found), it is skipped with a warning
   * and the server continues normally.
   * <p>
   * Failures will be logged but won't throw exceptions - the loader will
   * return an empty list if all providers fail or none are configured.
   */
