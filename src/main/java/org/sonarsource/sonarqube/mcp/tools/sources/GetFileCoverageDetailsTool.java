...
public class GetFileCoverageDetailsTool extends Tool {
  ...
  public GetFileCoverageDetailsTool(ServerApiProvider serverApiProvider) {
    super(SchemaToolBuilder.forOutput(GetFileCoverageDetailsToolResponse.class)
        ...
        .setDescription("Get line-by-line coverage information for a specific file, " +
        ...
        .addNumberProperty(FROM_PROPERTY, "First line to analyze (1-based, default: 1)")
        .addNumberProperty(TO_PROPERTY, "Last line to analyze (inclusive). If not specified, all lines are returned")
        .setReadOnlyHint()
        .build(),
  ...
  public Tool.Result execute(Tool.Arguments arguments) {
    ...
    try {
      var sourceLinesResponse = serverApiProvider.get().sourcesApi().getSourceLines(key, branch, pullRequest, from, to);
  ...
  private static GetFileCoverageDetailsToolResponse buildCoverageResponse(String key, @Nullable String filePath, SourceLinesResponse response) {
    ...
    var lineCoveragePercent = coverableLines.isEmpty() ?
      100.0 : (coveredLinesCount * 100.0 / coverableLines.size());
    ...
    var branchCoveragePercent = totalConditions == 0 ?
      100.0 : (coveredConditions * 100.0 / totalConditions);
    ...
    var uncoveredLines = sources.stream()
      ...
      .map(line -> new GetFileCoverageDetailsToolResponse.UncoveredLine(
        line.line(),
        stripHtmlTags(line.code())
      ))
      .toList();
  ...
  /**
   * Strip HTML tags from source code returned by SonarQube API.
   * The API returns code with syntax highlighting HTML tags like &lt;span class="k"&gt;.
   */
