...
public class AnalyzeListFilesTool extends Tool {
  ...
  public AnalyzeListFilesTool(SonarQubeIdeBridgeClient bridgeClient) {
    super(new SchemaToolBuilder()
      .setName(TOOL_NAME)
      .setDescription("Analyze files that have been modified in the current working directory using SonarQube for IDE. " +
        "This tool connects to a running SonarQube for IDE instance to perform code quality analysis on a list of files.")
      .addArrayProperty(LIST_FILES_PROPERTY, "string", "List of file paths to analyze")
      .build());
  ...
  public Result execute(Arguments arguments) {
    ...
    if (analysisResult.isEmpty()) {
      return Result.failure("Failed to start SonarQube for IDE analysis. Check logs for details.");
    }
  ...
  private String formatAnalysisResults(SonarQubeIdeBridgeClient.AnalyzeListFilesResponse results) {
    ...
    sb.append("1. Check SonarQube for IDE - issues are now displayed in your extension\n");
    sb.append("2. Hover over highlighted code for descriptions and quick fixes\n");
    sb.append("3. Ask the agent to fix the issues.\n");

    return sb.toString();
  }

  private String formatFinding(SonarQubeIdeBridgeClient.AnalyzeListFilesIssueResponse issue) {
