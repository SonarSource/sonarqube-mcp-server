...
class GetQualityGateBadgeToolTests {
  ...
  class MissingPrerequisite {
    ...
    void it_should_return_an_error_if_sonarqube_cloud_token_is_missing(SonarMcpServerTestHarness harness) {
      ...
      var result = mcpClient.callTool(new McpSchema.CallToolRequest(
        GetQualityGateBadgeTool.TOOL_NAME,
        Map.of(GetQualityGateBadgeTool.PROJECT_PROPERTY, "my-project")));
  ...
  class WithServer {
    ...
    void it_should_show_error_when_request_fails(SonarMcpServerTestHarness harness) {
      mockServer.stubFor(get(ProjectBadgesApi.QUALITY_GATE_PATH + "?project=" + URLEncoder.encode("my-project", StandardCharsets.UTF_8))
      ...
      assertThat(result)
        .isEqualTo(new McpSchema.CallToolResult("An error occurred during the tool execution: SonarQube answered with Error 500 on " + mockServer.baseUrl() + "/api" +
          "/project_badges/quality_gate?project=my-project", true));
    }
    ...
    void it_should_return_the_badge_svg(SonarMcpServerTestHarness harness) {
      mockServer.stubFor(get(ProjectBadgesApi.QUALITY_GATE_PATH + "?project=" + URLEncoder.encode("my-project", StandardCharsets.UTF_8))
        .willReturn(aResponse().withResponseBody(
          Body.fromOneOf(null, generateQualityGateSvg(), null, null)
        )));
      var mcpClient = harness.newClient(Map.of(
        "SONARQUBE_CLOUD_URL", mockServer.baseUrl(),
        "SONARQUBE_CLOUD_TOKEN", "token",
      ...
      assertThat(result)
        .isEqualTo(new McpSchema.CallToolResult(generateQualityGateSvg(), false));
    ...
    void it_should_return_the_badge_svg_with_optional_parameters(SonarMcpServerTestHarness harness) {
      ...
      var result = mcpClient.callTool(new McpSchema.CallToolRequest(
        GetQualityGateBadgeTool.TOOL_NAME,
        Map.of(
          GetQualityGateBadgeTool.PROJECT_PROPERTY, "my-project",
          GetQualityGateBadgeTool.BRANCH_PROPERTY, "feature",
          GetQualityGateBadgeTool.TOKEN_PROPERTY, "secret"
        )));
  ...
  private static String generateQualityGateSvg() {
    return """
