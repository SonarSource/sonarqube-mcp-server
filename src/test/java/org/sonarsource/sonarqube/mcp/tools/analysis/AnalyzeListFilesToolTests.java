...
class AnalyzeListFilesToolTests {
  ...
  @Test
  void it_should_have_correct_tool_name() {
    assertThat(underTest.definition().name()).isEqualTo("analyze_list_files");
  }

  @Test
  void it_should_have_correct_description() {
    assertThat(underTest.definition().description()).isEqualTo(
      "Analyze files that have been modified in the current working directory using SonarQube for IDE. " +
  ...
  class WhenBridgeIsAvailable {
    ...
    void it_should_return_failure_when_analysis_fails() {
      ...
      assertThat(result).isEqualTo(new McpSchema.CallToolResult("Failed to start SonarQube for IDE analysis. Check logs for details.", true));
    }
