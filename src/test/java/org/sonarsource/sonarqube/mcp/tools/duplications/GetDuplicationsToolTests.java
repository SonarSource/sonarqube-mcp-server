...
class GetDuplicationsToolTests {
  ...
  class WithSonarCloudServer {
    ...
    @SonarQubeMcpServerTest
    void it_should_return_duplications_with_branch_parameter(SonarQubeMcpServerTestHarness harness) {
      var duplicationsJson = """
        {
          "duplications": [
            {
      ...
      harness.getMockSonarQubeServer().stubFor(get(DuplicationsApi.DUPLICATIONS_SHOW_PATH + "?key=" + urlEncode("my_project:src/foo/Bar.php") + "&branch=" + urlEncode("feature/my_branch"))
        .willReturn(aResponse().withBody(duplicationsJson).withHeader("Content-Type", "application/json")));
      var mcpClient = harness.newClient(Map.of(
        "SONARQUBE_ORG", "org"
      ));
