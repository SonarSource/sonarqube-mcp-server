...
class UnifiedConfigValidatorTest {
  ...
  @Test
  void validateConfig_should_fail_when_config_is_null() {
    var result = UnifiedConfigValidator.validateConfig(null);
    
    assertThat(result.isValid()).isFalse();
    assertThat(result.message()).isEqualTo("Configuration cannot be null");
  }
  ...
  void validateConfig_should_succeed_for_valid_config() {
    var config = List.of(
      Map.of("name", "server1", "command", "/usr/bin/server"),
      Map.of("name", "server2", "command", "/usr/bin/other", "args", List.of("--port", "8080"))
    );
    
    var result = UnifiedConfigValidator.validateConfig(toMutableConfigs(config));
  ...
  void validateConfig_should_fail_when_name_is_blank() {
    var config = List.of(
      Map.of("name", "  ", "command", "/usr/bin/server")
    );
  ...
  void validateConfig_should_fail_on_duplicate_names() {
    var config = List.of(
      Map.of("name", "server", "command", "cmd1"),
      Map.of("name", "server", "command", "cmd2")
    );
  ...
  void validateConfig_should_fail_on_duplicate_namespaces() {
    var config = List.of(
      Map.of("name", "server1", "namespace", "shared", "command", "cmd1"),
      Map.of("name", "server2", "namespace", "shared", "command", "cmd2")
    );
  ...
  void validateConfig_should_use_name_as_namespace_for_uniqueness_check() {
    ...
    var config = List.of(
      Map.of("name", "server1", "command", "cmd1"),
      Map.of("name", "server2", "namespace", "server1", "command", "cmd2")
    );
  ...
  void validateConfig_should_fail_when_args_is_not_array() {
    var configs = List.of(
      new HashMap<>(Map.of("name", "server", "command", "cmd", "args", "not-an-array"))
    );
    
    var result = UnifiedConfigValidator.validateConfig(toMutableConfigs(configs));
  ...
  void validateConfig_should_fail_when_env_is_not_object() {
    var configs = List.of(
      new HashMap<>(Map.of("name", "server", "command", "cmd", "env", "not-an-object"))
    );
  ...
  void validateConfig_should_collect_multiple_errors() {
    var config = List.of(
      Map.of("command", "cmd1"),  // Missing name
      Map.of("name", "server")    // Missing command
    );
  ...
  @Test
  void validationResult_failure_single_should_set_message_and_errors() {
  ...
  @Test
  void validationResult_failure_multiple_should_summarize_message() {
  ...
  /**
   * Helper to convert immutable maps to mutable ones for testing.
   */
  @SuppressWarnings("unchecked")
  private List<Map<String, Object>> toMutableConfigs(List<? extends Map<String, ?>> configs) {
